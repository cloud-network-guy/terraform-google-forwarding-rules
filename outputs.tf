...
output "ip_addresses" {
  value = [for i, v in local.ip_addresses : (v.is_regional ? google_compute_address.default[v.index_key].address : google_compute_global_address.default[v.index_key].address)]
}
